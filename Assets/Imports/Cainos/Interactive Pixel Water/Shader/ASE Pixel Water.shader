		...
		Pass
		{
			...
			CGPROGRAM
				...
				float4 GaussianBlur( float4 GrabScreen, float2 TexelSize, float2 UV, float Blur )
				{
					    ...
					    for (int x = lower; x <= upper; ++x)
					    {
					        for (int y = lower; y <= upper; ++y)
					        {
					            ...
					            col += tex2Dlod(_CameraSortingLayerTexture, float4(UV + offset, 0, 0));
					        }
					    }
					    col /= kernelSum;
				...
				half4 frag( v2f IN  ) : SV_Target
				{
					...
					float2 screenUV = ase_grabScreenPos.xy / ase_grabScreenPos.w;
					float4 screenColor96 = tex2D(_CameraSortingLayerTexture, screenUV);
